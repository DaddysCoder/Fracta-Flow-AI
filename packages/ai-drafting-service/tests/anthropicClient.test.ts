...
import { runDraftGeneration } from "../src/anthropicClient";
...
describe("runDraftGeneration", () => {
  it("returns success and parses the response on a well-formed reply", async () => {
    ...
    const result = await runDraftGeneration(client, "claude-sonnet-4-5", prompt);
    expect(result).toEqual({
      outcome: "success",
      result: { draft: "Offer a visual countdown before transitions.", evidenceUsedIds: ["ev-1"] },
    });
  });
