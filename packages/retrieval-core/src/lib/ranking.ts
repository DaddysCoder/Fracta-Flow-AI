import { baseTokens, queryConceptGroups, queryTokens } from "./text-search";
import type { Chunk, KnowledgeDocument, RetrievalHit } from "./types";
...
function versionIntent(query: string) {
  const q = query.toLowerCase();
  return {
    asksHistory: /\b(what did|what was|previous version|old version|historical|former version|prior version|superseded version)\b/.test(q),
    asksCurrent: /\b(current|latest|effective|now|today)\b/.test(q),
  };
}
...
export function rankCandidates(query: string, candidates: RetrievalCandidate[], limit = 6): RetrievalHit[] {
  ...
  const tokenized = candidates.map(({ chunk, document }) => ({
    chunk,
    document,
    tokens: baseTokens(chunk.text),
    titleTokens: baseTokens(document.name),
  }));
  ...
  const { asksHistory, asksCurrent } = versionIntent(query);
  const queryTermSet = new Set(qTerms);
  const newestEffectiveDate = tokenized
    .filter(({ document, tokens, titleTokens }) =>
      !document.inspection.possibleSupersededLanguage &&
      document.effectiveDate &&
      [...tokens, ...titleTokens].some((token) => queryTermSet.has(token)),
    )
  ...
  const ranked = tokenized
    .map(({ chunk, document, tokens, titleTokens }) => {
      const titleTokenSet = new Set(titleTokens);
      const supportTokenSet = new Set([...tokens, ...titleTokens]);
      const conceptMatches = matchingConceptIndexes(supportTokenSet, concepts);
      ...
      let score = 0;
      const literalBodyMatches = new Set<string>();
      const expandedBodyMatches = new Set<string>();
      const literalTitleMatches = new Set<string>();
      const expandedTitleMatches = new Set<string>();

      for (const term of qTerms) {
        const tf = counts.get(term) ?? 0;
        if (tf) {
          const isLiteral = literalTerms.has(term);
          if (isLiteral) literalBodyMatches.add(term);
          else expandedBodyMatches.add(term);
        ...
        if (titleTokenSet.has(term)) {
          if (literalTerms.has(term)) literalTitleMatches.add(term);
          else expandedTitleMatches.add(term);
      ...
      if (document.inspection.possibleSupersededLanguage && !asksHistory) score -= 4.5;
      if (asksCurrent && document.inspection.possibleSupersededLanguage) score -= 3;
      if (asksCurrent && newestEffectiveDate && document.effectiveDate === newestEffectiveDate) score += 1.2;

      const literalMatches = new Set([...literalBodyMatches, ...literalTitleMatches]).size;
      const expandedMatches = new Set([...expandedBodyMatches, ...expandedTitleMatches]).size;
